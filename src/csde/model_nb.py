...
class NBInterceptModule(nn.Module):
    n_classes: int
    n_features: int
    ...
    def __call__(self, x, y, w=None):
        ...
        log_px_c_unsummed = NegativeBinomial2(mean=rates, concentration=concentrations).log_prob(x)
...
class NBIntercept(PPIAbstractClass):
    ...
    def fit(self, lambd_: Optional[Union[float, np.ndarray]] = None):
        if lambd_ is None:
    ...
    def get_lambda(
        ...
        if self.lambd_mode == "element":
            ...
            if isinstance(idx_to_optimize, int):
                return num[idx_to_optimize, idx_to_optimize] / denom[idx_to_optimize, idx_to_optimize]
            else:
    ...
    def _construct_contrast(self, feature_id: int, idx_a: int) -> np.ndarray:
        ...
        return np.hstack(
            [mu_contrast.flatten(), mu0_contrast, rho_contrast]
        )
    ...
    def idx_to_feat(self) -> np.ndarray:
        ...
        rho_identifier = np.arange(self.n_features)
        identifier = np.hstack([mu_identifier.flatten(), mu0_identifier, rho_identifier])
    ...
    def _get_param_id(
        ...
        if param_type == "mu":
            ...
            return n_params_mu + n_params_mu0 + feature_id
        return 0
    ...
    def hessian_fn(
        ...
        def process_hess(x, y, w=None):
            ...
            return jnp.block(
                [
                    [mu_mu, mu_mu0, mu_rho],
                    [mu_mu0.T, mu0_mu0, mu0_rho],
                    [mu_rho.T, mu0_rho.T, rho_rho],
                ]
            )
